...
import { detectInAppBrowser, isStandalonePWA } from '@/lib/inAppBrowser';
...
export default function Auth() {
  ...
  const showOAuthWarning = inApp.isInApp || standalone;
    ...
    <div className="flex-1 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          {showOAuthWarning && (
            <div className="rounded-md border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/30 p-3 text-sm flex gap-2">
              <AlertTriangle className="h-4 w-4 text-amber-600 shrink-0 mt-0.5" aria-hidden />
              <div>
                <p className="font-medium">
                  {inApp.isInApp ? t('auth.inapp_warning_title', language) : t('auth.standalone_warning', language)}
                </p>
                {inApp.isInApp && (
                  <p className="text-muted-foreground text-xs mt-1">{t('auth.inapp_warning_body', language)}</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">{t('auth.or', language)}</span>
            <Separator className="flex-1" />
          </div>
